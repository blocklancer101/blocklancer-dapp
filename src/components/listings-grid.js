...
class ListingsGrid extends Component {
  ...
  componentWillMount() {
    ...
    .then((networkId) => {
      return fetch(`https://raw.githubusercontent.com/blocklancer101/blocklancer-dapp/hide_list/hidelist_${networkId}.json`)
    })
    .then((response) => {
    ...
    .catch((error) => {
      if (error.message.indexOf("(network/artifact mismatch)") > 0) {
        console.log("The Blocklancer Contract was not found on this network.\nYou may need to change networks, or deploy the contract.")
      }
    })
